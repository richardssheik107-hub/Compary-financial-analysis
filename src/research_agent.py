...
from src.report_loader import load_company_documents, retrieve_relevant_passages, unsupported_source_note
...
def build_company_research_context(query_or_code: str, query: str = "") -> CompanyResearchContext:
    ...
    documents = load_company_documents(company_name, stock_code)
    ...
    company_profile = "；".join(summaries["business_segments"][:2])
    if not company_profile:
        company_profile = f"{company_name} 当前主要依据公开财务摘要进行初步经营研究，本地年报/研报资料暂未命中。"
...
def research_company(query: str) -> ResearchResult:
    ...
    one_line = (
        f"{context.company_name} 当前研究重点是收入增长、利润质量与经营现金流是否同向改善；"
        f"已取得收入 {revenue}、净利润 {profit}、经营现金流 {cashflow}。"
    )
    business = _join_or_infer(
        context.business_segments,
        f"{context.company_name} 的业务画像需要结合年报主营业务继续补充，当前先以财务数据判断经营质量。",
    )
    operating = (
        f"近年经营表现可先看三项：收入 {revenue}，营收同比 {revenue_yoy}；净利润 {profit}，净利润同比 {profit_yoy}；"
        f"经营现金流 {cashflow}。若三者同步改善，说明增长质量更扎实；若利润强但现金流弱，需要警惕回款或营运资本压力。"
    )
    profitability = (
        f"盈利质量方面，当前重点看利润增长是否来自主营业务，而不是一次性收益。"
        f"若后续补充毛利率、费用率和 ROE，可进一步判断公司是否具备持续赚钱能力。"
    )
    cash_quality = (
        f"现金流质量方面，经营现金流为 {cashflow}。它应与净利润 {profit} 对照看："
        "现金流长期弱于利润，通常意味着利润含金量不足；现金流长期覆盖利润，则经营质量更稳。"
    )
    growth = _join_or_infer(
        context.management_discussion + context.research_views,
        "成长性需要结合行业空间、公司产能/产品结构和管理层战略验证，不能只看单期增速。",
    )
    risks = _join_or_infer(
        context.risks,
        "主要风险包括需求波动、行业竞争、盈利能力下滑、现金流弱化和政策/周期变化。",
    )
...
def compare_companies_deep(query: str, agent_result: Any | None = None) -> ResearchResult | None:
    ...
    result = ResearchResult(
        ...
        business_overview=(
            f"{context_a.company_name}：{context_a.company_profile} "
            f"{context_b.company_name}：{context_b.company_profile}"
        ),
        operating_performance=(
            f"{context_a.company_name} 收入 {_metric(a_metrics, 'revenue')}、利润 {_metric(a_metrics, 'net_profit')}；"
            f"{context_b.company_name} 收入 {_metric(b_metrics, 'revenue')}、利润 {_metric(b_metrics, 'net_profit')}。"
            "对比时应优先看同一报告期、同一口径下的增长和盈利质量。"
        ),
        profitability_quality=(
            "盈利质量对比要看利润增长是否伴随收入增长，以及是否依赖一次性收益。"
            f"当前 {context_a.company_name} 净利润同比 {_metric(a_metrics, 'net_profit_yoy')}，"
        ...
        cashflow_quality=(
            ...
            "谁的现金流更能覆盖利润，谁的经营质量通常更稳。"
        ),
        growth_and_industry=(
            "成长性不能只看单期增速，还要看业务空间、竞争格局和管理层执行。"
            f"{context_a.company_name} 与 {context_b.company_name} 后续都应补充年报业务分部和研报行业观点。"
        ),
        major_risks=(
            f"{context_a.company_name} 风险：{_join_or_infer(context_a.risks, '需继续补充资料识别风险')} "
            f"{context_b.company_name} 风险：{_join_or_infer(context_b.risks, '需继续补充资料识别风险')}"
        ),
        tracking_checklist=[
            "两家公司是否处在同一报告期口径",
            "收入、利润、现金流是否同步改善",
...
def general_business_qa(query: str) -> ResearchResult:
    ...
    answer = " ".join(
        item
        for item in [
            analysis.plain_cashflow_summary,
    ...
    if not answer:
        answer = "这个问题需要结合收入、利润、现金流、资产负债和行业背景一起判断；如果补充具体公司，可以进一步做公司级研究。"
    ...
    result = ResearchResult(
        ...
        source_notes=[
            SourceNote(source_type="模型推理", title="通用经营问题", detail="未绑定具体公司，回答来自通用财务分析框架。")
        ],
        confidence_score=60,
